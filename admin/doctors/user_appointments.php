...
if (isset($_POST['update_status'])) {
    ...
    try {
        ...
        elseif ($new_status === 'Cancelled') {
            // Check if appointment was previously confirmed (need to deduct reward points)
            ...
            // If payment was made with reward points, refund them immediately
            if ($appointment['payment_method'] === 'reward_points') {
                $refund_points = $appointment['amount_paid'];
                $update_points = "UPDATE tbluser SET reward_points = reward_points + ? WHERE Id = ?";
                $stmt = $con->prepare($update_points);
                $stmt->bind_param("di", $refund_points, $appointment['user_id']);
                
                if (!$stmt->execute()) {
                    throw new Exception("Failed to refund reward points");
                }
                
                $_SESSION['success'] = "Appointment cancelled. " . number_format($refund_points, 2) . " reward points have been refunded to the user.";
                ...
                $message = "Your appointment has been cancelled. " . number_format($refund_points, 2) . " reward points have been refunded to your account.";
                $stmt->bind_param("iis", $appointment['user_id'], $appointment_id, $message);
                $stmt->execute();
            } else {
                // For card payments
                $_SESSION['success'] = "Appointment cancelled. Payment will be refunded within 24 hours.";
                
                // Insert notification for user
                $notification_query = "INSERT INTO notifications (user_id, appointment_id, message, created_at) VALUES (?, ?, ?, NOW())";
                $stmt = $con->prepare($notification_query);
                $message = "Your appointment has been cancelled. Your payment of " . number_format($appointment['amount_paid'], 2) . " Taka will be refunded within 24 hours.";
                $stmt->bind_param("iis", $appointment['user_id'], $appointment_id, $message);
                $stmt->execute();
            }
        } 
